...
def listado(agrupaciones, secciones, papeles, datos, orden, separar, activos, union, numerar, separar_cabeza, ancho_columnas, cabecera, archivo):
    ...
    pdf.ln(1)
    pdf.set_y(30)
    if separar == "Secciones":  # En caso de seleccionar separación por secciones
        for instru in secciones:
            # Construcción de la query para cada sección
            query = "SELECT " + ", ".join(datos) + " FROM actualizar_plantilla WHERE activo IN ('" + "', '".join(activos) + "')"
            query += " AND papel IN ('" + "', '".join(papeles) + "')"
            query += " AND agrupacion IN ('" + "', '".join(agrupaciones) + "')"
            query += " AND seccion = %s "
            query += orden_query
            # Creación de la tabla
            tabla = obtener_tabla_para_pdf(query, datos, numerar, union, (instru,), separar_cabeza)
            if tabla is None:
                continue
            else:
                # Si la tabla no está vacía, se añade al PDF
                pdf.set_font("Helvetica", "B", 15)
                instru_w = pdf.get_string_width(instru) + 6
                if pdf.get_y() + 22 > pdf.page_break_trigger:
                    pdf.add_page()
                pdf.cell(instru_w, 8, instru, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                insertar_tabla(tabla, pdf, ancho_columnas)
    elif separar == "Agrupaciones":  # En caso de seleccionar separación por agrupación
        for agrup in agrupaciones:
            ...
            if tabla is None:
                ...
                insertar_tabla(tabla, pdf, ancho_columnas)
    elif separar == "Papeles":  # En caso de seleccionar separación por papel
        for papel in papeles:
            # Construcción de la query para cada papel
            query = "SELECT " + ", ".join(datos) + " FROM actualizar_plantilla WHERE activo IN ('" + "', '".join(activos) + "')"
            query += " AND seccion IN ('" + "', '".join(secciones) + "')"
            query += " AND agrupacion IN ('" + "', '".join(agrupaciones) + "')"
            query += " AND papel = %s "
            query += orden_query
            # Creación de la tabla
            tabla = obtener_tabla_para_pdf(query, datos, numerar, union, (papel,))
            if tabla is None:
                continue
            else:
                # Si la tabla no está vacía, se añade al PDF
                pdf.set_font("Helvetica", "B", 15)
                papel_w = pdf.get_string_width(papel) + 6
                if pdf.get_y() + 22 > pdf.page_break_trigger:
                    pdf.add_page()
                pdf.cell(papel_w, 8, papel, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                insertar_tabla(tabla, pdf, ancho_columnas)
    else:
        # Construcción de la query para todas las secciones seleccionadas
        query = f"SELECT " + ", ".join(datos) + " FROM actualizar_plantilla WHERE activo IN ('" + "', '".join(activos) + "')"
        query += " AND agrupacion IN ('" + "', '".join(agrupaciones) + "')"
        query += " AND seccion IN ('" + "', '".join(secciones) + "')"
        query += " AND papel IN ('" + "', '".join(papeles) + "')"
...
def insertar_tabla(tabla, pdf, anchos):
    ...
    elemento(tabla[0][:-2], pdf, longitudes, altura=9, fill=True)
    for miembro in tabla[1:]:
        if miembro[-2] and miembro[-1]:
            pdf.set_text_color(0, 0, 0)
        elif miembro[-2] and not miembro[-1]:
            pdf.set_text_color(0, 255, 0)
        elif not miembro[-2] and not miembro[-1]:
            pdf.set_text_color(0, 0, 0)
        else:
            pdf.set_text_color(0, 0, 0)
        elemento(miembro[:-2], pdf, longitudes)
    pdf.ln()
    pdf.set_text_color(0, 0, 0)
    return
...
if __name__ == "__main__":
    listado(["Orquesta"], [], [], ["surname", "name", "activo", "programa_anterior"],
            "surname", "Secciones", "", "Apellidos,Nombre", False, True, "80, 30, 80", 
            "\n\n\n", "web_airesdeespaña")